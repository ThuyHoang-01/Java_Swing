...
public class QuanLyLaoDong_GUI extends JPanel implements ActionListener, ItemListener, MouseListener {
	...
	public QuanLyLaoDong_GUI() {
		...
		add(lbMaLD);
		// them button tim kiem
		btnTimKiem = new JButton("Tìm kiếm");
		btnTimKiem.setBounds(712, 192, 120, 35);
	...
	public void deleteDataTable() {
		while(tableLaoDong.getRowCount() > 0) {
			modelLaoDong.removeRow(0);
		}
	}
	...
	public boolean kiemtraDuLieu() {
		...
		if (!(txtHoTen.getText().matches("\\p{L}+(\\s\\p{L}+)*"))) {
			JOptionPane.showMessageDialog(this, "Tên không được phép có kí tự đặc biệt");
			foCus(txtHoTen);
			return false;
		}
		...
		if (ngaySinh.getDate() == null) {
			JOptionPane.showMessageDialog(this, "Mời chọn ngày sinh");
			return false;
		}
		SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		...
		if (cbGioiTinh.getSelectedIndex() == 0) {
			if (Period.between(ld, LocalDate.now()).getYears() > 60) {
				JOptionPane.showMessageDialog(this, "Người lao động nam phải nhỏ hơn 60 tuổi");
				return false;
			}
		} else {
			if (Period.between(ld, LocalDate.now()).getYears() > 55) {
				JOptionPane.showMessageDialog(this, "Người lao động nữ phải nhỏ hơn 55 tuổi");
				return false;
			}
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src.equals(btnThem)) {
			...
			if (kiemtraDuLieu()) {
				...
				int yesNo = JOptionPane.showConfirmDialog(this, "Bạn có muốn thêm?", "Message",
		...
		if (src.equals(btnCapNhat)) {
			...
			if (kiemtraDuLieu()) {
				...
				int yesNo = JOptionPane.showConfirmDialog(this, "Bạn có muốn cập nhật?", "Message",
						JOptionPane.YES_NO_OPTION);
				if (yesNo == JOptionPane.YES_OPTION) {
					if (dao_qlld.suaLD(ld)) {
						// cap nhat table
						tableLaoDong.setValueAt(ld.getTenLaoDong().toUpperCase(), row, 1);
	...
	public void timKiem() {
		deleteDataTable();
		listLD = null;
		listLD = dao_qlld.timKiemLaoDong(txtMaLD.getText().trim(), txtTenLD.getText().trim());
		try {
			loadDataTable(listLD);
		} catch (Exception e) {

...
public class FrmDanhMucBangDia extends JFrame implements ActionListener, MouseListener {
	...
	private JTextField txtGhiChu, txtGia, txtSL;
	...
	private void buildUI() {
		...
		pCen.add(btnTim = new JButton("Tìm kiếm"));
	...
	private BangDia revertSachFromTextFields() {
		...
		return new BangDia(maBD, tenBD, theLoai, hangSX, tinhTrang, ghiChu,donGia, Integer.parseInt(sl));
	}
		 ...
		 table.scrollRectToVisible(table.getCellRect(index, index, true));
			...
			if (row != -1) {
				...
				if (hoinhac == JOptionPane.YES_OPTION) {
					if (QuanLyBangDia.xoaBangDia(row)) {
						tableModel.removeRow(row);
						txtMess.setText("�?ã xóa");
						clearTextFields();
					}
				}
			} else
				txtMess.setText("Bạn cần phải ch�?n một cd cần xóa");
		} else if (o.equals(btnThem)) {
			BangDia cd = revertSachFromTextFields();
			String[] row = { maBD, tenBD, theLoai, hangSX, tinhTrang, ghiChu,gia,sl + "\n" };
			if (QuanLyBangDia.themBangDia(cd)) {
			...
			String maCDOld = txtMaBangDia.getText().trim();
			BangDia CDNew = revertSachFromTextFields();
			if (QuanLyBangDia.capNhatBangDia(maCDOld, CDNew)) {
				txtMess.setText("Cập nhật thành công");
				int row = table.getSelectedRow();
				tableModel.setValueAt(txtMaBangDia.getText(), row, 0);
				tableModel.setValueAt(txtTenBangDia.getText(), row, 1);
				tableModel.setValueAt(txtTheLoai.getText(), row, 2);
				tableModel.setValueAt(txtHangSX.getText(), row, 3);
				tableModel.setValueAt(cboTinhTrang.getSelectedItem().toString(), row, 4);
				tableModel.setValueAt(txtGhiChu.getText(), row, 5);
				tableModel.setValueAt(txtGia.getText(), row, 6);
				tableModel.setValueAt(txtSL.getText(), row, 7);
			} else
				txtMess.setText("Cần ch�?n cd để cập nhật");
		} else if (o.equals(btnLuu)) {
			Database.SaveDatabaseBD(QuanLyBangDia.getDsBangDia());

...
public class MonHoc_GUI extends JFrame implements ActionListener, MouseListener {
	...
	public MonHoc_GUI() {
		...
		Database.connect();
		for (BoMon bm : BoMonChuQuan_Dao.getall()) {
			cboBoMonChuQuan.addItem(bm.getMaMon());
		}
		...
		JLabel lblName;
		b8.add(lblName = new JLabel("Họ tên sv: Trần Minh Nhứt massv:18092291"));
	...
	private void DocDuLieuVaoBang() {
		for (MonHoc mh : MonHoc_Dao.getall()) {
			dataModel.addRow(new Object[] { mh.getMaMon(), mh.getTenMon(), mh.getSoTiet(), mh.getBm().getMaMon() });
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnThem)) {
			if(kiemtra()) {
				BoMon bm =new BoMon(cboBoMonChuQuan.getSelectedItem().toString());
				MonHoc mh = new MonHoc(txtMaMonHoc.getText(),txtTenMonHoc.getText(),bm,Integer.parseInt(txtSoTiet.getText()));
				MonHoc_Dao.themMH(mh);
				dataModel.addRow(new Object[] { mh.getMaMon(), mh.getTenMon(), mh.getSoTiet(), mh.getBm().getMaMon() });
		...
		if (o.equals(btnXoa)) {
			...
			if(r==-1) {
				...
				if(hoi==0) {
					MonHoc_Dao.xoaMH(txtMaMonHoc.getText());
	...
	private boolean kiemtra() {
		...
		//if(!(txtTenMonHoc.getText().length() >0 &&txtTenMonHoc.getText().matches("[a-AZ-z]*") )) {
		//	JOptionPane.showMessageDialog(this, "tên lỗi ");
		//	return false;
		//}
		if(!(txtSoTiet.getText().length()>0 && Integer.parseInt(txtSoTiet.getText())>15&&Integer.parseInt(txtSoTiet.getText())<90)) {
			JOptionPane.showMessageDialog(this, "số tiết lỗi >15 và <90 ");
			return false;
		}
		return true;
	}
